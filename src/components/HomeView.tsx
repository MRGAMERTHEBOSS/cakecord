  ...
  return (
    <div className="relative py-8">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 pb-16">
        <div className="rounded-[28px] border border-zinc-900 bg-gradient-to-br from-zinc-900/70 via-zinc-900/50 to-zinc-950/70 p-8 md:p-10 shadow-[0_24px_60px_rgba(0,0,0,0.24)]">
          <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] gap-8 items-start">
            <div className="text-left">
              <span className="text-[11px] font-mono font-bold tracking-[0.2em] text-pink-500 uppercase">
                / FAST DEPLOYMENT
              </span>
              <h3 className="mt-3 text-2xl md:text-3xl font-extrabold text-white tracking-tight">
                Ready in under two minutes
              </h3>
              <p className="mt-3 text-sm md:text-base text-zinc-400 leading-relaxed max-w-xl">
                Drop the essentials into your server once and let CakeCord keep the celebration flow running automatically.
              </p>
              <div className="mt-6 flex flex-wrap gap-3">
                <div className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950/50 px-3 py-1.5 text-xs font-semibold text-zinc-300">
                  <Shield size={13} className="text-pink-400" />
                  <span>No code needed</span>
                </div>
                <div className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950/50 px-3 py-1.5 text-xs font-semibold text-zinc-300">
                  <Compass size={13} className="text-purple-400" />
                  <span>Friendly defaults</span>
                </div>
                <div className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950/50 px-3 py-1.5 text-xs font-semibold text-zinc-300">
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-5 text-left">
              ...
              <div className="mt-4 space-y-3">
                {[
                  { command: "/settings channel text_channel: #announcements", label: "Pick a celebratory channel" },
                  { command: "/settings role assign_role: @Birthday Star", label: "Add a role for special members" },
                  { command: "/birthdays set month: 09 day: 24 timezone: Europe/London", label: "Register your first birthday" }
                ].map((item) => (
                  <div key={item.command} className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-3">
                    <p className="font-mono text-xs text-pink-400">{item.command}</p>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 pb-8">
        <div className="rounded-[28px] border border-pink-500/20 bg-gradient-to-r from-pink-500/10 via-purple-500/10 to-indigo-500/10 px-6 py-8 md:px-8 md:py-10 text-left shadow-[0_15px_40px_rgba(236,72,153,0.08)]">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              <span className="text-[11px] font-mono font-bold tracking-[0.2em] text-pink-400 uppercase">
                / START TODAY
              </span>
              <h3 className="mt-2 text-2xl md:text-3xl font-extrabold text-white tracking-tight">
                Launch CakeCord On Your Server Today
              </h3>
              <p className="mt-2 max-w-2xl text-sm md:text-base text-zinc-400 leading-relaxed">
                Start celebrating members automatically with clean birthday tracking, daily announcements, and server engagement boosts.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button
                onClick={onInviteClick}
                className="px-5 py-2.5 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 text-sm font-semibold text-white shadow-lg shadow-pink-500/20 transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
              >
                Add CakeCord
              </button>
              <button
                onClick={() => onTabChange("premium")}
                className="px-5 py-2.5 rounded-full border border-zinc-800 bg-zinc-950/50 text-sm font-semibold text-zinc-200 transition-all hover:bg-zinc-900 cursor-pointer"
              >
                Free Forever
              </button>
            </div>
